...
将已同步的本地 `main` 合并到 `personal/main` 时，应按功能语义逐项解决冲突：吸收上游修复，同时保留仍然有效的个人功能和本机兼容性修改。不得通过整体选择 `main`、`origin/main` 或上游一侧来覆盖 `personal/main`。只有在上游已经提供等价实现，或者经过验证确认个人修改已经失效、不再需要时，才删除该修改。

仅属于单台机器的路径、密钥和运行配置应保存在 `.env` 等已忽略的本地文件中，不应合并到 `personal/main`。如果修改是产品在该类环境上正常运行所必需的兼容性代码，则应保留在工作分支和 `personal/main` 中，并通过测试验证。合并冲突无法明确判断时应停止晋级，查明两边修改的意图后再继续；不得以快速消除冲突为目的丢弃任一侧行为。
