...
This fork keeps `upstream/main` as the read-only upstream baseline, local `main` as the clean synchronization branch, and `origin/main` as the fork's mirror of local `main`. `personal/main` is the personal complete product line: it may contain multiple selected, tested `feature/*` and `fix/*` changes, and it is updated only by merging those work branches. Product work belongs on one branch per work item: `feature/*` for features, `fix/*` for bug fixes, or the documented `docs/*`, `test/*`, `chore/*`, and `refactor/*` equivalents. Develop work on those branches, not directly on `main` or `personal/main`.

After a fresh clone, enable the repository hooks with `.\scripts\setup_git_workflow.ps1`. Use `.\scripts\sync_main.ps1` to fetch `upstream`, fast-forward local `main`, and push the synchronized main to `origin`. Use `.\scripts\new_work_branch.ps1 -Name <name> -Kind feature|fix` to create a work branch from the synchronized `main`. Merge tested work branches into `personal/main` when they are ready to become part of the complete personal product line. Full details are in [docs/git-workflow.md](docs/git-workflow.md). The hooks block direct commits on `main` and `personal/main`, allow merge commits into `personal/main`, and block all pushes to `upstream`; they do not replace review or testing.
